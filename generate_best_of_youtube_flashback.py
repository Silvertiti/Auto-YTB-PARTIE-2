...
STREAMERS_FILE = "streamers_flashback.txt"
MIN_DURATION_SECONDS = 1200  # 10 minutes
# MAX_CLIPS_PER_STREAMER supprimé
...
def get_clips_for_broadcaster(headers, broadcaster_id, start_time, end_time, limit=20):
...
def main():
    ...
    # --- 1. Récupération de l'ID du jeu GTA V (pour filtrage) ---
    gta_game_id = get_game_id(headers, "Grand Theft Auto V")
    if not gta_game_id:
        print("⚠️ ID GTA V introuvable, le filtrage par jeu sera désactivé.")
    else:
        print(f"🎮 GTA V Game ID: {gta_game_id}")
    ...
    # --- 3. Récupération des clips par STREAMER ---
    streamers_names = []
    if os.path.exists(STREAMERS_FILE):
        with open(STREAMERS_FILE, "r", encoding="utf-8") as f:
            streamers_names = [line.strip() for line in f if line.strip()]

    if not streamers_names:
        print(f"❌ Veuillez ajouter des noms dans {STREAMERS_FILE}")
        return

    print(f"🔍 Recherche des clips pour {len(streamers_names)} streamers...")
    
    raw_clips = []
    for name in streamers_names:
        uid = get_user_id(headers, name)
        if uid:
            print(f"  > Checking {name}...", end="\r")
            c = get_clips_for_broadcaster(headers, uid, start_str, end_str, limit=50)
            raw_clips.extend(c)
        else:
            print(f"  > ⚠️ Introuvable : {name}")
    print("") # Newline

    # --- 4. Filtrage "GTA V" et "Flashback" ---
    clips_pool = []
    print(f"🧹 Filtrage 'GTA V' + 'Flashback' parmi {len(raw_clips)} clips...")
    for clip in raw_clips:
        # Filtre Jeu (si on a l'ID)
        if gta_game_id and clip.get('game_id') != gta_game_id:
            continue
            
        title = clip['title'].lower()
        # Filtre Titre (Optionnel mais demandé pour "Flashback")
        # On garde si ça parle de Flashback
        if "flashback" in title:
            clips_pool.append(clip)
            
    print(f"✅ Clips retenus : {len(clips_pool)}")
    ...
    for clip in unique_clips:
        ...
        if download_clip(clip['url'], raw_path):
            ...
            if standardize_video(raw_path, std_path, streamer_name):
                ...
                # --- ON INCRÉMENTE LE COMPTEUR (Juste pour info) ---
                streamer_counts[streamer_name] = streamer_counts.get(streamer_name, 0) + 1
    ...
    # --- Ecriture Formatée pour Copy-Paste ---
    desc_path = os.path.join(OUTPUT_DIR, "description_youtube.txt")
    with open(desc_path, "w", encoding="utf-8") as f:
        f.write(f"TITRE :\n{metadata['title']}\n\n")
        f.write(f"DESCRIPTION :\n{metadata['description']}\n")
        
    print(f"📄 Titre et Description sauvegardés dans : {desc_path}")
    # On garde le json au cas où
